...
class MyTokenObtainPairSerializer(TokenObtainPairSerializer):
    ...
    @classmethod: This line indicates that the following function is a class method, which means it belongs to the class itself and not to an instance (object) of the class. As a result, the method receives the class (cls) as its first argument, rather than an instance (self).
...
class RegisterSerializer(serializers.ModelSerializer):
    ...
    # When you set write_only=True on a field in a serializer, it means that this field will be accepted when creating or updating an instance, but it will not be included in the serialized representation of that instance when it is returned in a response.
    ...
    def validate(self, attrs):  # 'attrs' is a dictionary that contains the validated data for the fields of the serializer.
    ...
    def create(self, validated_data):  # validated_data is a dictionary that contains the data that has been validated and is ready to be used for creating or updating an object.
        ...
        user.set_password(validated_data['password'])  # The set_password method ensures that the password is hashed using a secure hashing algorithm before saving it in the database.
        user.save()
...
# The to_representation method in DRF serializers allows you to customize the serialized output of model instances.
# By overriding this method, you can modify the default representation, add additional fields, or include nested serializers.
# In your example, the method is used to include the serialized data of the related user instance in the output of the ProfileSerializer.
...
# }
    ...
    def to_representation(self, instance):
        ...
        response['user'] = UserSerializer(instance.user).data  # a user key is added to response dictionary which contains user serializer data. instance.user is accessing the user attribute of Profile model.
        return response

# DRF uses the serializer to validate the input email.
# You don’t need a model — this is purely input validation and logic triggering

class PasswordResetSerializer(serializers.Serializer):
    email = serializers.EmailField()

class CategorySerializer(serializers.ModelSerializer):
    # SerializerMethodField is a special field in Django REST Framework that lets you add a custom (computed) field to your serialized output — even though that field doesn't exist in your model.

    #  How SerializerMethodField() Works
    # 1. You define the field in your serializer like this:
    # my_field = serializers.SerializerMethodField()

    # 2. Then you create a method named get_<field_name>:
    # def get_my_field(self, obj):
    #     return "some computed value"

    post_count = serializers.SerializerMethodField()  # post_count is added as a key in json.
...
class PostSerializer(serializers.ModelSerializer):
    # This field (comments) will be a list of objects — serialize each one using CommentSerializer, and put them in an array. (many=True) is responsible for storing json objects in an array.
    comments = CommentSerializer(many=True)  # This is a nested serializer. We can access comments from the post serializer.
