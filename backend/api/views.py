...
# DRF’s CreateAPIView needs two things:
# 1. A serializer (serializer_class) — this defines:
#         a. The model to use (via Meta.model)
#         b. The fields to save
#         c. How to validate and create the object
# 2. A queryset — needed only for DRF’s internal mechanisms (e.g., introspection, hooks like get_queryset())

class RegisterView(generics.CreateAPIView):
    queryset = api_models.User.objects.all()
    serializer_class = api_serializer.RegisterSerializer
    permission_classes = (AllowAny,)  # permission_classes always expects an iterable, so assign it like this (AllowAny,) or [AllowAny], (IsAuthenticated,) or [IsAuthenticated]

class ProfileView(generics.RetrieveUpdateAPIView):
    permission_classes = (AllowAny,)
    serializer_class = api_serializer.ProfileSerializer

    # We didn't use lookup_field = [pk] because lookup_field only works if the object you're retrieving is the same model as your queryset. But we need to retrieve data first from User table and then use that data to retrieve Profile data. And kwargs['user_id'] is used in User model.

    def get_object(self):  # DRF uses this method to find the object that should be retrieved/updated.
        user_id = self.kwargs['user_id']  # Assumes the URL contains a path like /profile/<user_id>/. In Django class-based views (like DRF views), self.kwargs is a dictionary that contains URL parameters captured from the route.
